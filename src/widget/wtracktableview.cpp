...
void WTrackTableView::loadTrackModel(QAbstractItemModel* pNewModel, bool restoreState) {
    ...
    // Column order is draggable and persisted per model. Widths remain
    // controlled by the in-skin weights, not transient pixel drag-resizing.
...
void WTrackTableView::loadSelectedTrackToGroup(const QString& group, bool play) {
    ...
    if (!mixxx::deckload::allowed(group, m_pConfig)) {
        return;
    }
