...
// Echo, Flanger, and some other effects load with their internal metaknob at
// zero. In that state the unit dry/wet knob can be turned up while producing
// almost no audible effect. Give a newly enabled effect a useful depth while
// preserving any non-zero depth the DJ has already chosen.
PioneerDDJFLX6.ensureFocusedFxHasAudibleDepth = function() {
    const group = PioneerDDJFLX6.focusedFxGroup();
    if (engine.getParameter(group, "meta") <= 0.001) {
        engine.setParameter(group, "meta", 0.5);
...
PioneerDDJFLX6.beatFxOnOffPressed = function(_channel, _control, value) {
    ...
    if (toggleEnabled) {
        PioneerDDJFLX6.ensureFocusedFxHasAudibleDepth();
    }
